...
class SmallPictClient:
    """
    Synchronous SmallPict API client for image optimization, CDN purging, and quota tracking.
    ...
    def __enter__(self) -> "SmallPictClient":
        return self
    ...
    def optimize(
        ...
        **kwargs: Any,
        ...
        """
        Compress and transcode an image to modern web formats (AVIF, WebP).

        :param source: Raw bytes, file path, URL, file-like object, or PIL Image.
        :param options: Structured OptimizeOptions instance (optional).
        :param kwargs: Keyword arguments to construct OptimizeOptions dynamically.
        :return: OptimizeResult with Edge CDN URL and compression statistics.
        """
        if options is None:
        ...
        try:
            res = self._request("POST", "/v1/optimize", json_data=payload, idempotency_key=options.idempotency_key)
    ...
    def get_quota(self) -> QuotaResponse:
        ...
        res = self._request("GET", "/v1/quota")
    ...
    def purge_cdn(
        ...
        :param urls: List of asset URLs or a single URL to purge.
        :param purge_type: PurgeType.URL or PurgeType.ALL.
        :return: PurgeResponse confirmation.
        """
        url_list: List[str] = []
        ...
        res = self._request("POST", "/v1/purge", json_data=payload)
    ...
    def get_job_status(self, job_id: str) -> JobStatusResult:
        ...
        res = self._request("GET", f"/v1/optimize/status?job_id={job_id}")
